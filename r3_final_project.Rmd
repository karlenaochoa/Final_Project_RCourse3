...
#I like this intro for functional programming.
...
d1_raw <- d1 %>% #Created a new variable, pid, based on row number
  ...
  #it might be helpful to put these in order for clarity. select(3:4, 7:114, 115:116)
...
#I really enjoyed that you added the long format above, it really shows how efficient writing a function can be.
```{r tsc_new}
...
#I definitely appreciate the meaningful function names.
```{r}
#GROUP 1
demo1 <- d1_raw %>% 
  get_demo_info
...
tsc1_long <- tsc1_long %>% # You could use the magrittR double pipe "%<>%" instead of assigning back to the same df
gather(item, response, -1:-7) %>% 
  separate(item, c(NA, "item"), sep = "_") %>% 
  mutate(scale = "tsc") %>% 
  select(1:7, 10, 8:9)# same suggestion as above select(1:7, 8:9, 10)
...
#subtract 1 from all the tsc columns
tsc2 <- subtract1(tsc2)#it may be helpful to mention why you would want to do this, oh I see you do it below!
...
gather(item, response, -1:-7) %>% 
  separate(item, c(NA, "item"), sep = "_") %>% 
  mutate(scale = "tsc") %>% 
  select(1:7, 10, 8:9) #select(1:7, 8:9, 10)
...
#When I run this, I get "computation failed in 'stat_bin()' and I went to check out the plots and there's no data for the gender4 and gender5 plot. 
```
...
#as you noted, these last two lines don't seem to be running properly and it's returning -Inf
```